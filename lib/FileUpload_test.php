...
class FileUpload extends Component {
    ...
    function __construct($uploadName = null) {
        ...
        if (isset($_FILES['Posts']['name'][$this->uploadName][0])) {
            $this->isXhr = false;

            if ($_FILES['Posts']['error'][$this->uploadName][0] === UPLOAD_ERR_OK) {
                $this->fileName = $_FILES['Posts']['name'][$this->uploadName][0];
                $this->fileSize = $_FILES['Posts']['size'][$this->uploadName][0];
            } else {
                $this->setErrorMsg($this->errorCodeToMsg($_FILES['Posts']['error'][$this->uploadName][0]));
            }
        } elseif (isset($_SERVER['HTTP_X_FILE_NAME']) || isset($_FILES['Posts']['name'][$this->uploadName][0])) {
            $this->isXhr = true;
    ...
    private function saveForm($path) {
        if (move_uploaded_file($_FILES['Posts']['tmp_name'][$this->uploadName][0], $path))
            return true;
        return false;
    }
